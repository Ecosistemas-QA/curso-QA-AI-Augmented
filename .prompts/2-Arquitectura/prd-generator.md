...
---
**Requisito previo:** Se debe haber completado primero el `business-model.md` y `market-context.md`(opcional). Si no es asi, detener la ejecucion de este prompt y sugerir al usuario la ejecucion de `.prompts\1-Constitucion\business-model.md` .
...
Para comenzar, leer los documentos de la Fase 1: `business-model.md` y `market-context.md`.
...
2.  Si el proyecto utiliza Jira, ofrece utilizar el **MCP de Atlassian** para extraer y analizar las Épicas y Historias de Usuario existentes, generando un resumen del estado actual.
3.  **Si es una aplicación web:** Sugiere que utilice una herramienta de captura o el **MCP de Playwright** para mapear los flujos de usuario actuales ("Happy Paths") y documentarlos como requisitos existentes.
